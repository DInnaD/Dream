 //callInstaAPI: function(){
	   ...
	   c = new String(Math.round(weather.currentTemp));
...
<!--nav class="blue" role="navigation">
  <div class="nav-wrapper my-nav-padding">
			<ul class="right hide-on-med-and-down">
        <li><i class="fa fa-refresh fa-2x my-cursor" id="re"></i></li>
      </ul>
  </div>
</nav-->
<!--script>
  ...
  data: {
    access_key: '50874a59333d4555564a50befbeffed0',
    query: 'Poltava'
  },
  dataType: 'json',
  success: function(apiResponse) {
    console.log(`Current temperature in ${apiResponse.location.name} is ${apiResponse.current.temperature}℃`);
	  var c = Math.round(apiResponse.current.temperature);
	  
  var galleryFeed3 = new Instafeed({
  get: "user",
  userId: 6715816580,
  accessToken: "6715816580.1677ed0.2e945d7bd1ae403cbdcca139dbc1b450",
  resolution: "standard_resolution",
  ...
  after: function() {    
    // disable button if no more results to load
    if (!this.hasNext()) {
      btnInstafeedLoad3.setAttribute('disabled', 'disabled');
    }
  },
});
galleryFeed3.run();

var btnInstafeedLoad3 = document.getElementById("btn-instafeed-load3");
btnInstafeedLoad3.addEventListener("click", function() {
  galleryFeed3.next()
});
  }
});
       ...
       <div class="intro-body">
            <div class="container">
                 <div class="row">                    
                	...
                	<div id="userFeed3" class="gallery row no-gutter"></div>
			<button id="btn-instafeed-load3" class="btn">Load more</button>  
                    </div>
                </div>
	     </div>
	</div>
    ...
    $.each(images, function(index, image) {
      ...
      $(image).addClass('animated flipInX');
		...
		success: function(response) {
			...
			var weatherUrl = `http://api.weatherstack.com/current?access_key=50874a59333d4555564a50befbeffed0&query=Poltava`;
//https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&appid=7353c4ca1f3017f3868f78fac0070c3e
...
//var c = Math.round((weather.feelslike - 32) / 1,8);
var c = Math.round(current.feelslike); /*nauchys' perevodyty v cyfru i == */		      
var nextButton = document.getElementById('next-button');    
	      console.log(c); 
	     // console.log(f);
$('.feelslike_c').html(c)  + '<a class="cel"> ºC</a>');
//$('.feelslike_f').html(f)  + '<a class="cel"> ºF</a>');          
var userFeed3 = new Instafeed({// http://instagram.pixelunion.net/  ORhttps://api.instagram.com/v1/users/self/media/liked?access_token=ACCESS-TOKEN  
get: 'user',
...
@media only screen and (max-width: 432px) {
  ...
  let app = {
    ...
    lat: "",
    long: "",
    key: "ffc64f46165aa2a5cd91269cc33dc6f7",
    spinner: document.querySelector('.loader'),
    card: document.querySelector('.custom-container'),
    baseURL: "https://api.openweathermap.org/data/2.5/weather?",
    msg: null,
    galleryFeed3: new Instafeed({
  get: "user",
  userId: 6715816580,
  accessToken: "6715816580.1677ed0.2e945d7bd1ae403cbdcca139dbc1b450",
  resolution: "standard_resolution",
...
btnInstafeedLoad3.addEventListener("click", function() {
  ...
  //ajax request
  app.updateForecast = () => {
    const url = `${app.baseURL}lat=${app.lat}&lon=${app.long}&appid=${app.key}`;
    fetch(url)
    .then((resp) => {
       // Transform the data into json
      let check = resp.clone().json();
      return check;
    })
    .then(function(data) {
      ...
      weatherData.c = (weatherData.temp - 273.15).toFixed(1) + '\xB0' + "C";
      weatherData.f = ((1.8 * (weatherData.temp - 273)) + 32).toFixed(1) + '\xB0' + "F";
      weatherData.temp = weatherData.c;
      app.setC(weatherData);

      console.log(cacheWeatherData.name);
    }).then(() => {
      app.getForecast();
      app.showForecast();
    }).catch(err => {
    ...
    localforage.clear().then(function() {
      if(data){
       Object.keys(data).forEach((key) => {
         localforage.setItem(key, data[key]);
       })
     } else {
       app.msg = "did not work"
     }
 ...
 app.getC = () => {
    localforage.iterate((val, key, iN) => {
      cacheWeatherData[key] = val;
    }).then(() => {
      console.log(cacheWeatherData);
    })
  }
  ...
  document.getElementById("temp").addEventListener('click', (e) => {
     if(weatherData.temp !== weatherData.f){
       weatherData.temp = weatherData.f;
     } else if(weatherData.temp !== weatherData.c){
       weatherData.temp = weatherData.c;
     }
    app.card.querySelector('.cardTemp').textContent = weatherData.temp;
  });
  ...
  app.showForecast = () => {
   app.bool = true;  
    ...
    localforage.length().then((length) => {
      ...
      if(length > 1){
        app.stopLoader(app.bool);
        app.c = weatherData.temp - 273.15.toFixed(1);
        app.f = ((1.8 * (weatherData.temp - 273)) + 32).toFixed(1);
  ...
  ***********/
  ...
  const getLocal = (position) => {
   app.lat = position.coords.latitude;
   app.long = position.coords.longitude;
   localforage.setItem("lat", app.lat);
   localforage.setItem("lon", app.long);
    
   app.updateForecast();
   app.showForecast();
  }
  
  const geoErrors = (error) => {
    console.log("unable to retrieve your location")
  }
  
  navigator.geolocation.getCurrentPosition(getLocal, geoErrors).then(() => app.stopLoader(app.bool))
});
			</script-->
  <!--script>
document.addEventListener("DOMContentLoaded", () => {
   ...
   app.bool = true;  
    ...
    localforage.length().then((length) => {
      ...
      if(length > 1){
        ...
        app.card.querySelector('.cardName').textContent = weatherData.name !== undefined ? weatherData.name: "Please Refresh";
        app.card.querySelector('.cardIcon').src = weatherData.icon;
        let d = new Date();
        app.card.querySelector('.cardDT').textContent = d.toDateString() + " " + d.toLocaleTimeString();
        app.card.querySelector('.cardDescription').textContent = weatherData.description;
        app.card.querySelector('.cardPressure').textContent = `Pressure: ${weatherData.pressure}`;
        app.card.querySelector('.cardHumidity').textContent = `Humidity: ${weatherData.humidity}%`;
        app.card.querySelector('.cardTemp').textContent = weatherData.temp;
        app.card.querySelector('.cardWindSpeed').textContent =`Wind Speed: ${weatherData.wind.speed}, Wind Degree: ${weatherData.wind.deg}`;
      } 
    })
    }, 1200); 
  }
